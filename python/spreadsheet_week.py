    ...
    for food in foods:
        if hour in range(open_hour, close_hour):
            if food not in menu:
                menu.append(food)
            if elapsed_day <= 7:
                if food in hours_week[hour]:
                    hours_week[hour][food] = hours_week[hour][food] + 1
                else:
                    hours_week[hour][food] = 1
for hour in hours_week:
    for food in menu:
        if food in hours_week[hour]:
            hours_week[hour][food] = hours_week[hour][food] / 7
        else:
            hours_week[hour][food] = .05
...
for menu_item in menu: #construct each bar
    bar = []
    for order_hour in hours_week:
        if menu_item in hours_week[order_hour]:
            if hours_week[order_hour][menu_item] > max_val:
                max_val = hours_week[order_hour][menu_item]
            if hours_week[order_hour][menu_item] < .05:
                bar.append(.05)
            else:
                bar.append(hours_week[order_hour][menu_item])
        else:
            bar.append(.05)
    r = random.random()
    g = random.random()
...
plt.legend()   
plt.savefig('../public/avg_week.png')
