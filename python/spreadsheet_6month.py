    ...
    for food in foods:
        if hour in range(open_hour, close_hour):
            if food not in menu:
                menu.append(food)
            if elapsed_day <= 31 * 6:
                if food in hours_6months[hour]:
                    hours_6months[hour][food] = hours_6months[hour][food] + 1
                else:
                    hours_6months[hour][food] = 1
for hour in hours_6months:
    for food in menu:
        if food in hours_6months[hour]:
            hours_6months[hour][food] = hours_6months[hour][food] / (31*6)
        else:
            hours_6months[hour][food] = .05
...
ax.set_title('Average Orders/Hour per 6 Months')
ax.yaxis.set_major_locator(tick.MultipleLocator(1))
max_val = 0
for menu_item in menu: #construct each bar
    bar = []
    for order_hour in hours_6months:
        if menu_item in hours_6months[order_hour]:
            if hours_6months[order_hour][menu_item] > max_val:
                max_val = hours_6months[order_hour][menu_item]
            if hours_6months[order_hour][menu_item] < .05:
                bar.append(.05)
            else:
                bar.append(hours_6months[order_hour][menu_item])
        else:
            bar.append(.05)
    r = random.random()
    g = random.random()
...
plt.legend()   
plt.savefig('../public/avg_6months.png')
